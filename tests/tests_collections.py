...
VERSIONFILE = pathlib.Path(pathlib.Path(__file__).parent,'dv_frankenstein_complete_api_output_all_versions.json')
with open(VERSIONFILE, encoding='utf-8') as w:
    VERSIONDATA = json.load(w)
...
class TestDvCollection(unittest.TestCase):
    def setUp(self):
        ...
        self.dv_coll.get_study_info = MagicMock(return_value=c.StudyMetadata(study_meta=APIDATA, all_versions=VERSIONDATA))
...
class TestStudyMetadata(unittest.TestCase):
    def setUp(self):
        self.teststudy = c.StudyMetadata(study_meta=APIDATA, all_versions=VERSIONDATA)
